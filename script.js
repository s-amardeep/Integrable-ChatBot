onload = function(){
    // outputs a javascript object from the parsed json
        ...
        var chat = { 
            messageToSend: '',   //Interface with frontend, any input and output on screnn will be one with this func
            messageResponses: [
            ...
            init: async function() {
                this.chatTree = new ChatTree(); //Creating new chat tree
                await this.chatTree.init(); //Initilization
                this.cacheDOM(); //Tooking refernces
                this.bindEvents(); //Puting some events when something is typed in chat area
                await this.render();
            },
            cacheDOM: function() {
            ...
            render: async function() { //Main Func which take i/p from http and sending to chat tree which will send data and again with this func it will be displayed on the http screen
                this.scrollToBottom();
                if (this.messageToSend.trim() !== '') {
                    var template = Handlebars.compile( $("#message-template").html()); //Templates
                    var context = {
                    ...
                    setTimeout(function() {
                        ...
                        this.scrollToBottom();
                    }.bind(this), 1000); //Artificial Delay
